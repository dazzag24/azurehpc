...
arm_template="deploy.yaml"

cat <<EOF >$arm_template
...
if [ "$vnet_resource_group" != "$resource_group" ]; then
    error "not handling case where external vnet is used"
...
- type: Microsoft.Network/publicIPAddresses
  ...
  properties:
    ...
    dnsSettings:
      domainNameLabel: $resource_vm_name$uuid_str
EOF
            fi
...
- type: Microsoft.Network/networkInterfaces
  ...
  name: ${resource_vm_name}VMNic
  location: $location
EOF
            if [ "$resource_pip" = "true" ]; then
                cat <<EOF >>$arm_template
  dependsOn:
  - Microsoft.Network/networkSecurityGroups/$nsg_name
...
EOF
  ...
  properties:
    ipConfigurations:
    - name: ipconfig$resource_vm_name
      properties:
        privateIPAllocationMethod: Dynamic
        subnet:
          id: $resource_subnet_id
        publicIPAddress:
          id: /subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Network/publicIPAddresses/$public_ip_address
    networkSecurityGroup:
      id: /subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Network/networkSecurityGroups/$nsg_name
    enableAcceleratedNetworking: $resource_an
- type: Microsoft.Compute/virtualMachines
  apiVersion: "2019-07-01"
  name: $resource_vm_name
  location: $location
  dependsOn:
  - Microsoft.Network/networkInterfaces/${resource_vm_name}VMNic
  tags: {}
  properties:
    hardwareProfile:
      vmSize: $resource_vm_type
    networkProfile:
      networkInterfaces:
      - id: /subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Network/networkInterfaces/${resource_vm_name}VMNic
    storageProfile:
      osDisk:
        ...
        diskSizeGb: 32
      imageReference:
        publisher: $(echo $resource_image | cut -d':' -f1)
        offer: $(echo $resource_image | cut -d':' -f2)
        sku: $(echo $resource_image | cut -d':' -f3)
        version: $(echo $resource_image | cut -d':' -f4)
    osProfile:
...
- type: Microsoft.Compute/virtualMachineScaleSets
  ...
  tags: {}
  sku:
    name: $resource_vm_type
    capacity: $resource_instances
  properties:
    overprovision: true
    upgradePolicy:
      mode: manual
    virtualMachineProfile:
      ...
      networkProfile:
        ...
        - name: ${resource_name}${uuid_str}Nic
          properties:
            primary: "true"
            ipConfigurations:
            - name: ${resource_name}${uuid_str}IPConfig
              properties:
                subnet:
                  id: $resource_subnet_id
    singlePlacementGroup: true
    platformFaultDomainCount: $resource_fault_domain_count
EOF
